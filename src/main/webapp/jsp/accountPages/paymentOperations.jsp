...
<div class="cardsOp">
    ...
    <form method="post" action="<c:url value="/bank24/PaymentOperations"/>">
        <select name="idFromCard" class="chargeCard" required oninvalid="this.setCustomValidity(<fmt:message key="HINT_SELECT_CARD"/>)" oninput="setCustomValidity('')">
        ...
        <select   name="idToCard" class="chargeCard" required oninvalid="this.setCustomValidity(<fmt:message key="HINT_SELECT_CARD"/>)" oninput="setCustomValidity('')">
            <option disabled selected value=""><fmt:message key="SELECT_CARD"/></option>
            <c:forEach items="${sessionScope.accountList}" var="accountElem">
                <option value="${accountElem.account.id}">${accountElem.type} ${accountElem.account.accountCode} ${accountElem.account.currentBalance} ${accountElem.account.currency}</option>
            </c:forEach>
        ...
        <input type="text" class="form-control" name="operationSumm" value="" required oninvalid="this.setCustomValidity(<fmt:message key="HINT_ENTER_AMOUNT"/>)" oninput="setCustomValidity('')" pattern="[\d]+[\.]?([\d]{1,2})?"/><br/>
    ...
    <form method="post" action="<c:url value="/bank24/PaymentOperations"/>">>
        ...
        <input type="text" class="form-control" name="idToCard" value=<fmt:message
